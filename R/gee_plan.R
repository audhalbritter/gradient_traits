  ...
  tar_target(
    ...
    command = {
      ...
      tryCatch({
        ...
        # Get the most recent image (or you can specify a date range)
        viirs_image <- viirs_collection$filterDate("2020-01-01", "2023-12-31")$median()
        
        # Select the growing season length band
        # VNP22Q2 bands: 'Onset_Greenness_Increase_1', 'Onset_Greenness_Maximum_1', 
        # 'Onset_Greenness_Decrease_1', 'Onset_Greenness_Minimum_1', 'EVI_Minimum_1', 
        # 'EVI_Maximum_1', 'EVI_Amplitude_1', 'EVI_Area_1'
        # Growing season length can be calculated as: Maximum - Minimum onset dates
        
        # Calculate growing season length (days between greenness increase and decrease)
        ...
        extracted_df <- rgee::ee_as_sf(extracted)
